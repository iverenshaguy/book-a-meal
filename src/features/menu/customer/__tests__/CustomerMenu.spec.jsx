...
describe('CustomerMenu', () => {
  ...
  it('should render Customer Menu component correctly', () => {
    const shallowWrapper = shallow(<CustomerMenu user={customer} fetchMenu={jest.fn()} />);
  ...
  it('should start fetching more menu items when Menu Component triggers loadMoreMenu function', () => {
    ...
    const comp = (
      <Provider store={store}>
        <CustomerMenu user={customer} fetchMenu={fetchMenuMock} />
      </Provider>
    );

    const wrapper = mount(comp, rrcMock.get());

    wrapper.find('Connect(CustomerMenuItems)').props().loadMoreMenu();
  ...
  it('should render a message when shop is closed', () => {
    ...
    expect(toJson(wrapper)).toMatchSnapshot();
    expect(wrapper.find('p.notif').text()).toEqual(
      'Ordering is only available between 8:30am and 4:00pm. Please check back later.'
    );
  ...
  describe('Order Creation', () => {
    ...
    it('should remove orderItem from Basket when delete icon is clicked', () => {
      ...
      wrapper.find('Cart').dive().find('Link.remove-order').dive().find('button.remove-order').simulate('click');
