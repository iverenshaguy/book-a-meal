...
describe('OrderPill', () => {
  it('should render Caterer OrderPil correctly', () => {
    const wrapper = shallow(<OrderPill order={caterersOrdersObj.orders[0]} user={caterer} />);

    expect(toJson(wrapper)).toMatchSnapshot();
  });

  it('should render Customer OrderPill correctly', () => {
    const wrapper = shallow(<OrderPill order={customersOrdersObj.orders[0]} user={customer} />);

    expect(toJson(wrapper)).toMatchSnapshot();
  });

  it('should render Customer OrderOill correctly and add ... for more than one meal', () => {
    const wrapper = shallow(<OrderPill order={customersOrdersObj.orders[1]} user={customer} />);
  ...
  it('should render Customer OrderPill correctly when status started', () => {
    const wrapper = shallow(<OrderPill order={customersOrdersObj.orders[3]} user={customer} />);
