...
describe('SideNav', () => {
  ...
  it('should render correctly when caterer tab is active', () => {
  ...
  it('should toggle sidenav and handle link click when caterer link is clicked', () => {
    const toggleSideNavMock = jest.fn();
    const pushMock = jest.fn();
  ...
  it('should close open SideNav when close icon is clicked', () => {
    const toggleSideNavMock = jest.fn();
    const wrapper = mount(
      <SideNav user={caterer} logout={jest.fn()} open active="orders" toggleSideNav={toggleSideNavMock} />,
      rrcMock.get()
    );
