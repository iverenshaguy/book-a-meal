...
import { formHelpers, formErrorCount, arrayToObject } from 'src/features/common/utils/';
...
export class Form extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.type !== state.type) {
      ...
      if (type === 'addMeal') {
        values.imageUrl = 'https://res.cloudinary.com/iverenshaguy/image/upload/v1532540264/bookameal/default-img.jpg';
      }
  ...
  validateForm = () => {
    ...
    } else if (!formErrorArrayLength && validateRequiredFields(requiredFields, this.state.values)) {
      this.setState({
        formValid: true,
      });
    }
  ...
  submitter = () => {
    ...
    const { type, meal, token, email } = this.props;
  ...
  /**
   * @memberof Form
   * @returns {React.ComponentClass} Form
   */
  renderForm = () => {
    const { type, submitting, meal, uploading } = this.props;
  ...
  render() {
    ...
    return (
      <div>
        {submitting && (
          <div className="modal-preloader text-center">
            <MiniPreloader />
          </div>
        )}
        {!submitting && (
