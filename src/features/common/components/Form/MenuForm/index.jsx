...
import { mealObjPropTypes, metadataProps } from 'src/features/common/utils/proptypes';
 ...
 * @extends Component
 * @class MenuForm
 * @returns {React.ComponentClass} MenuForm
 */
export class MenuForm extends Component {
  ...
  static getDerivedStateFromProps(props) {
    const error = props.submitError && props.submitError.date ? props.submitError.date.value : props.submitError;
  ...
  /**
   * @memberof MenuForm
   * @returns {React.ComponentClass} SearchBox
   */
  renderDate = () => (
  ...
  /**
   * @memberof MenuForm
   * @returns {React.ComponentClass} MenuFormMeals
   */
  renderMeals = () =>
    ...
    return (
      <Fragment>
        ...
        {!isFetching && !submitting && meals.length !== 0 && (
          <button type="button" className="btn btn-pri btn-block" onClick={this.submitMenu}>
            SAVE MEAL OPTIONS
          </button>
        )}
      </Fragment>
