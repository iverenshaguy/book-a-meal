...
describe('Modal', () => {
  ...
  it('should render correctly when type prop is orderSuccessMsg', () => {
    ...
    expect(toJson(shallowWrapper)).toMatchSnapshot();
    expect(shallowWrapper.find('p.text-center').text()).toEqual(
      'Thank you for your order. Your belly will be filled up shortly'
    );
  });
  ...
  it('should render connected Modal component correctly', () => {
    const comp = (
      <Provider store={store}>
        <WrappedModal type="addMeal" open toggleModal={jest.fn()} />
      </Provider>
    );
