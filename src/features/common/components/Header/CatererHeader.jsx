...
import { toggleSideNav as toggleSideNavActions } from 'src/features/common/data/actions';

/**
 * @exports
 * @function CatererHeader
 * @param {object} props
 * @returns {React.ComponentClass} CatererHeader
 */
export const CatererHeader = ({ currentDay, showTime, dateType, toggleSideNav, updateCurrentDate }) => {
  const date =
    dateType === 'menu' ? moment(currentDay).format('dddd[,] Do MMMM YYYY') : moment().format('dddd[,] Do MMMM YYYY');
...
export default connect(mapStateToProps, { toggleSideNav: toggleSideNavActions })(CatererHeader);
