 ...
 * @param {object} props
 * @returns {React.ComponentClass} CustomerHeader
 */
const CustomerHeader = ({ logout, user, active }) => {
  const links = (
    <Fragment>
      <Link to="/" className={`${active === 'menu' && 'active'}`}>
        Menu
      </Link>
      <Link to="/orders" className={`${(active === 'orders' || active === 'customerOrderDetails') && 'active'}`}>
        Orders
      </Link>
